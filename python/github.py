...
# On Windows, prevent subprocesses from opening a new console window. The
# CREATE_NO_WINDOW flag suppresses the visible terminal that would
# otherwise appear when running commands such as npm or electron-builder.
if os.name == "nt":
    try:
        CREATE_NO_WINDOW = subprocess.CREATE_NO_WINDOW
    except AttributeError:
        # Fallback constant for older Python versions
        CREATE_NO_WINDOW = 0x08000000  # CREATE_NO_WINDOW from Windows API
else:
    CREATE_NO_WINDOW = 0
import os
import threading
    ...
    def run(self):
        try:
            ...
            if cmd_list:
                ...
                cmd_list[0] = resolved
            try:
                # Prevent a console window from appearing on Windows by passing
                # the CREATE_NO_WINDOW flag. On non-Windows platforms this
                # flag is zero so it has no effect.
...
class BuildWorker(QThread):
    ...
    def run(self):
        try:
            ...
            for i, command in enumerate(self.commands):
                ...
                try:
                    # Hide the console window on Windows by using the CREATE_NO_WINDOW flag.
                    process = subprocess.Popen(
                        cmd_list,
                        stdout=subprocess.PIPE,
                        stderr=subprocess.STDOUT,
                ...
                if self._is_running:
                    # Wait for the command to finish and capture the return code
                    process.wait()
                    # Perform cleanup of lingering processes that may lock files
    ...
    def _cleanup_build_locks(self):
        """
        Attempt to terminate any lingering processes that may lock the output
        directory or resources generated by electron-builder. In addition to
        electron.exe and node.exe, electron-builder spawns auxiliary executables
        such as app-builder.exe, signtool.exe, 7za.exe and elevate.exe during
        packaging. If these processes outlive the main build command, they can
        prevent successive builds from removing or overwriting files. This
        method iterates through running processes and kills those whose names
        contain known identifiers.
        """
        try:
            import psutil as _psutil
            # List of substrings identifying processes to terminate
            targets = [
                "electron.exe", "electron", "makeyourlifeeasier.exe",
                "node.exe", "node", "app-builder", "app-builder.exe",
                "signtool.exe", "7za.exe", "elevate.exe", "winsw.exe", "sudo.exe"
            ]
            for proc in _psutil.process_iter(['pid', 'name']):
                try:
                    name = proc.info.get('name', '')
                    lower_name = name.lower() if name else ''
                    if any(t in lower_name for t in targets):
                        proc.kill()
                except Exception:
                    # Ignore any process we cannot access or kill
                    continue
        except Exception:
            # psutil may not be available or accessible
            pass
    ...
    def run_command_sync(self, command, cwd=None, require_project=True, check_git=False):
        ...
        try:
            ...
            if cmd_list:
                resolved = _resolve_executable(cmd_list[0])
                cmd_list[0] = resolved
            # Use CREATE_NO_WINDOW to prevent flashing a console window on Windows.
