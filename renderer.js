  ...
  async function loadPage(key) {
    ...
    switch (key) {
      ...
      case 'bios':
        ...
        break;
        case 'updater':
  setHeader('Ενημέρωση Εφαρμογής');
  content.appendChild(await buildUpdaterPage());
  break;
      default:
        content.textContent = '';
    }
...
async function buildUpdaterPage() {
  const container = document.createElement('div');
  container.className = 'card';
  ...
  versionContainer.style.padding = '1rem';
  versionContainer.style.background = 'var(--sidebar-bg)';
  versionContainer.style.borderRadius = '8px';
  versionContainer.style.border = '1px solid var(--border-color)';
  ...
  try {
    ...
    versionContainer.innerHTML = `
      <div style="display: flex; justify-content: space-between; align-items: center;">
        <div>
          <strong>Τρέχουσα Έκδοση:</strong> v${versionInfo.version}
        </div>
    ...
    versionContainer.innerHTML = `
      <div style="color: var(--error-color);">
        <strong>Σφάλμα:</strong> Δεν μπορώ να λάβω πληροφορίες έκδοσης
      </div>
    `;
  }
