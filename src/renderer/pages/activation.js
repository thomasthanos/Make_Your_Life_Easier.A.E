...
const toast = createNotifier('activate_autologin');
import { uiText } from '../ui-text.js';

import { registerDownload, attachDownloadUI, downloadStore, finishDownload, updateDownloadPhase, getActiveDownload } from '../downloads.js';
...
async function downloadAndRun(button, config) {
    ...
    if (getActiveDownload(config.storeKey)) return;
    ...
    registerDownload(storeKey, downloadId, { name: config.name, followUp: true });
    ...
    return new Promise((resolve) => {
        attachDownloadUI(storeKey, (data) => {
            switch (data.status) {
                ...
                case 'complete': {
                    ...
                    ui.setProgress(null);
                    updateDownloadPhase(storeKey, config.runningText);

                    window.api.openFile(data.path)
                        .then((result) => {
                            if (result && result.success) {
                                finishDownload(storeKey);
                                toast(uiText('tool_opened'), { type: 'success', title: config.name });
                                ...
                                button.textContent = originalText;
                                finishDownload(storeKey, config.runFailMsg);
                        ...
                        .catch(() => {
                            button.textContent = originalText;
                            finishDownload(storeKey, config.runErrorMsg);
                ...
                case 'error':
                    ...
                    fail(uiText("download_failed", "Download failed"));
                    resolve();
                    break;
            }
        });
        ...
        try {
            ...
            finishDownload(storeKey, uiText('download_failed'));
            fail(uiText("download_failed", "Download failed"));
            resolve();
        }
    });
...
function downloadAndRunActivate(button, ui, idleText) {
    return downloadAndRun(button, {
        ...
        name: uiText("activation", "Activation"),
        preparingText: uiText("preparing_activation", "Preparing activation..."),
        downloadingText: uiText("downloading_activation", "Downloading activation script..."),
        runningText: uiText("running_activation", "Running activation script..."),
        startedText: uiText("activation_started", "Activation Started"),
        runFailMsg: uiText("activation_failed", "Failed to run activation script"),
        runErrorMsg: uiText("activation_error", "Error running activation script")
    });
}
...
function downloadAndRunAutologin(button, ui, idleText) {
    return downloadAndRun(button, {
        ...
        name: uiText("autologin", "Auto Login"),
        preparingText: uiText("preparing_autologin", "Preparing auto login..."),
        downloadingText: uiText("downloading_autologin", "Downloading auto login tool..."),
        runningText: uiText("running_autologin", "Running auto login setup..."),
        startedText: uiText("autologin_started", "Auto Login Started"),
        runFailMsg: uiText("autologin_failed", "Failed to run auto login tool"),
        runErrorMsg: uiText("autologin_error", "Error running auto login tool")
    });
}
...
export async function buildActivateAutologinPage(translations, _settings) {
    const pageText = translations.activation_ui || {};
    ...
    kicker.textContent = pageText.page_kicker || 'Windows Access Center';
    ...
    readyText.textContent = pageText.ready || 'Ready';
    ...
    const statItems = [
        { value: '2', label: pageText.tools_label || 'tools' },
        { value: '1', label: pageText.admin_count_label || 'admin action' }
    ];
    ...
    noticeText.textContent = pageText.security_notice || 'Tools are downloaded on demand and launched locally.';
    ...
    sectionTitle.textContent = pageText.section_title || 'Activation & Sign-in';
    const sectionDescription = document.createElement('p');
    sectionDescription.className = 'activation-section-description';
    sectionDescription.textContent = pageText.section_description || 'Choose a Windows access tool to download and run.';
    const sectionCount = document.createElement('span');
    sectionCount.className = 'activation-section-count';
    sectionCount.textContent = '2';
    sectionCount.setAttribute('aria-label', `2 ${pageText.tools_label || 'tools'}`);
    ...
    const readyHint = pageText.ready_hint || 'Ready — one click to download and run.';
    ...
    grid.appendChild(createActivationCard({
        ...
        badge: pageText.admin_badge || 'Admin required',
        badgeTone: 'admin',
    ...
    grid.appendChild(createActivationCard({
        ...
        badge: pageText.local_badge || 'Local setup',
        badgeTone: 'info',
